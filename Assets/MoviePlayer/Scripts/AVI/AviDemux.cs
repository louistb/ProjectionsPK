...
namespace MP.AVI
{
	...
	public class AviDemux : Demux
	{
		...
		// not normally used, but audio stream can be the first one too in AVI file
		public const uint ID_00wb = 0x62773030; // audio chunk, stream 0
		public const uint ID_01dc = 0x63643130; // "compressed" video chunk, stream 1
		public const uint ID_01db = 0x62643130; // "uncompressed" video chunk, stream 1
		...
		public override void Init (Stream sourceStream, LoadOptions loadOptions = null)
		{
			...
			if (avi.strhVideo != null) {
				...
				if(videoStreamInfo.frameCount == 0) videoStreamInfo.frameCount = (int)avi.strhVideo.dwLength;
		...
		private bool ProcessAviList (RiffParser rp, uint fourCC, int length)
		{
			...
			if (fourCC == ID_movi) {
				...
				//idx1EntryOffset = 8; // XXX There's on clip that needed this
			}
		...
		private void ProcessAviChunk (RiffParser rp, uint fourCC, int unpaddedLength, int paddedLength)
		{
			...
			case ID_indx: // OpenDML index
				...
				if (streamId == ID_00dc || streamId == ID_00db || streamId == ID_01dc || streamId == ID_01db) {
					avi.videoIndex = index;
				} else if (streamId == ID_01wb || streamId == ID_00wb) {
		...
		private void PrepareVideoStream ()
		{
			...
			// This is a bit hackish, but we need to make the buffer 420B larger, because
			// VideoDecoderMJPEG may need to add standard MJPEG Huffman table (mjpegDefaultDHT)
			// when the frame doesn't include its own. When mjpeg coded adds it, it uses the
			// same buffer for max efficiency (no buffer to buffer copy, no extra heap allocations).
			rawVideoBuf = new byte[maxChunkLength + 420];
