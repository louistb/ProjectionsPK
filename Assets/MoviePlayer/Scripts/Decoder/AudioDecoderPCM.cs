...
namespace MP.Decoder
{
	...
	public class AudioDecoderPCM : AudioDecoder
	{
		...
		public const uint FORMAT_UNCOMPRESSED_FLOAT = 0x0000FFFE;
		...
		public AudioDecoderPCM (AudioStreamInfo streamInfo)
		{
			...
			if (streamInfo.audioFormat != FORMAT_UNCOMPRESSED &&
			    streamInfo.audioFormat != FORMAT_UNCOMPRESSED_FLOAT &&
			...
			if ((streamInfo.audioFormat == FORMAT_UNCOMPRESSED_FLOAT && bytesPerChannelSample != 4) ||
			    (streamInfo.audioFormat != FORMAT_UNCOMPRESSED_FLOAT && bytesPerChannelSample > 2)
			) {
				throw new ArgumentException ("Only 8bit and 16bit_le int, and 32bit float audio is supported. " + (bytesPerChannelSample * 8) + "bits given");
			}
		}
		...
		public override void DecodeNext (float[] data, int sampleCount)
		{
			...
			try {
				...
				for (int c = 0; c < channels; c++) {
					...
					else if (streamInfo.audioFormat == FORMAT_UNCOMPRESSED_FLOAT) {
						Buffer.BlockCopy(buf, 0, data, 0, samplesActuallyRead * streamInfo.sampleSize);
					}
