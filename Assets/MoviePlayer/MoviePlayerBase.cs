...
public abstract class MoviePlayerBase : MonoBehaviour
{
	...
	/// <summary>
	/// Gets and sets the video stream framerate.
	/// 
	...
	public float framerate {
		...
		set {
			// the setter is only available for MoviePlayer class
			if (this is MoviePlayer && movie != null && movie.demux != null && movie.demux.videoStreamInfo != null) {
				var mp = this as MoviePlayer;
				mp.videoTime = value == 0 ? 0 : mp.videoTime * mp.framerate / value;
				movie.demux.videoStreamInfo.framerate = value;
			}
		}
	}
