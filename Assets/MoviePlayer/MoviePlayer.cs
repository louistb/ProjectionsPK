...
public class MoviePlayer : MoviePlayerBase
{
	...
	/// <summary>
	/// If TRUE the movie will play in reverse and 
	/// FALSE the movie will play normal.
	/// Use this for rewinding.
	/// </summary>
	...
	protected bool HandlePlayheadMove ()
	{
		...
		if (seekedByVideoFrame) {
			...
			videoTime += isReverse ? -Time.deltaTime : Time.deltaTime;
		}
	...
	protected void HandleLoop ()
	{
		...
		bool isEndOfVideo = isReverse ? (videoTime <= 0f) : (videoTime >= movie.demux.videoStreamInfo.lengthSeconds);

		if (isEndOfVideo)
		{
			if (loop)
			{
				// seek to the beginning or the end of the video
				videoTime = isReverse ? movie.demux.videoStreamInfo.lengthSeconds : 0f;
